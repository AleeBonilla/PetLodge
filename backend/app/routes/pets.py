...
blp = Blueprint("pets", __name__, url_prefix="/api/v1/pets", description="Gestión de mascotas")
...
class PetList(MethodView):
    ...
    @blp.doc(summary="Registrar mascota", description="Registra una nueva mascota para el dueño autenticado")
    def post(self, data):
    ...
    @jwt_required()
    @blp.doc(summary="Listar mis mascotas", description="Lista todas las mascotas del dueño autenticado")
    def get(self):
        owner_id = int(get_jwt_identity())
...
class PetDetail(MethodView):
    @jwt_required()
    @blp.doc(summary="Ver detalle de mascota", description="Obtiene los datos de una mascota específica")
    def get(self, pet_id):
        owner_id = int(get_jwt_identity())
    ...
    @jwt_required()
    @blp.doc(summary="Eliminar mascota", description="Soft delete de una mascota (no permitido si tiene reservas activas)")
